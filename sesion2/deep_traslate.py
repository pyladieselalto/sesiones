...
class GoogleTranslator(ParentTranslator):
    ...
    def __init__(self, source="auto", target="en"):
        ...
        super(GoogleTranslator, self).__init__(base_url=self.__base_url,source=self._source,target=self._target,element_tag='div',element_query={"class": "result-container"},hl=self._target,sl=self._source,q=None)
...
class PonsTranslator(ParentTranslator):
    ...
    def __init__(self, source="french", target="english"):
        ...
        if self._validate_languages([source.lower(), target.lower()]):
            self._source = self._map_language_to_code(source.lower())
            self._target = self._map_language_to_code(target.lower())
        super().__init__(base_url=self.__base_url,source=self._source,target=self._target,element_tag='div',element_query={"class": "target"})
    def _map_language_to_code(self, language):
        """
        @param language: type of language
        @return: mapped value of the language or raise an exception if the language is not supported
...
if __name__ == '__main__':
    ...
    # ])
    res_pons = PonsTranslator(source="english", target="spanish").translate(payload='good')
    res_goo = GoogleTranslator(source="en", target="es").translate(payload='good')
    print(f"PONS good: {res_pons}")
    print(f"GOOGLE good: {res_goo}")